...
rm -rf "${iso_file}"
mkdir -p "${script_dir}/download-cache"
...
echo "${manifest_bosh}" | bosh int - --path /releases | grep url | awk '{print $2}' >> "${download_dir}/downloads.txt"
echo "${stemcell_url}" > "${download_dir}/downloads.txt"
...
echo "$manifest_cf" | bosh int - --path /releases | grep url | awk '{print $2}' >> "${download_dir}/downloads.txt"
echo "https://bosh.io/d/stemcells/bosh-warden-boshlite-ubuntu-trusty-go_agent?v=$stemcell_version" >> "${download_dir}/downloads.txt"
...
wget --no-http-keep-alive -P "${download_dir}" -c -i "${download_dir}/downloads.txt"
